...
- **Branch policy (owner decisions — follow exactly):**
  ...
  - The `ReportOutputButton` (mailto feedback) lives on both lines; it is not part of the censorship layer. **Never hardcode the owner's personal email (or any personal PII) into `REPORT_EMAIL` or anywhere else in the source.** This repo is public with GitHub Pages auto-deploying `main` on every push — anything hardcoded here is instantly public, in the source *and* in the shipped bundle. `REPORT_EMAIL` stays `""` (button hides itself) until the owner explicitly gives a dedicated project/support address to use instead.
